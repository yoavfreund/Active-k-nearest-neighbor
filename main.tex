...
\begin{itemize}
\item We are given a finite dataset of $n$ points, and have the ability to query the labels of any of them. The response to the query $x$ +1 or -1, chosen according the conditional probability function $\eta(x)$. When two or more queries are made on the instance $x$, the responses need not be the same, in other words, the labels are not persistant. 
...
\yoav{Looks suspicious: $|X_B| \geq k$ implies that the query set $\Gamma(B)$ has size at least $k$. The size of the query set should depend on the bias, not on the size of $X_B$}
\begin{thm}
...
\yoav{Shouldn't this be $|\eta(x)| > c$?} 
\begin{defn}
...
First, I find it more intuitive to talk about the probability associated with the ball, rather than the level, which is an exponentially quantized version of the probability. 

I am looking to characterize the behaviour on $x$ when using the true distribution. Suppose this true distribution on $X$ is a uniform density over a ball. In this case there is a 1-to-1 relation between probability and radius and I will use $r(p), p(r)$ to denote the mappings in both directions.

Fix $\gamma>0$ and define the sets 
$$ A_{\gamma} = \left\{x | \eta(x)\geq \gamma \right\},
 \;\;\;B_{\gamma} = \left\{x | \eta(x)\leq -\gamma \right\}$$
 
 I use $\B_{p}(x)$ to denote the set of balls with probability at $p$ that contain $x$, similarly $\B_{\leq p}(x)$ is the set of balls that contain $x$ and have probability at most $p$.
 
 I posit that you can characterize the least obscured $p$ and the largest resolved $p$ as follows:
 \begin{itemize}
     \item The largest resolved $p$ for $x$ is $p(r'/2)$ where $r'$ is the radius of the largest ball centered at $x$ that is contained in $A_\gamma$ if $\eta(x)>\gamma$ or $B_\gamma$ if $\eta(x)<-\gamma$ 
 \item The smallest obscuring $p$  for $x$ is $p(r'')$ where $r''$ is the largest radius such that for any $s<r''$, $\B_{s}(x)$ contains at least one ball whose polarity is correct.
 \end{itemize}

\section{Analysis for a fixed distribution}
\newcommand{\P}{{\cal P}}
...
Our goal is to upper bound the number of examples the algorithm needs needs to query before the prediction of the algorithm on $x$ converges to the correct label (and will not change with additional samples).

Given the distribution $\P$ and an edge parameter $\gamma>0$ we partition the balls in $\B$ into three 
subsets: $\B^+ = $
