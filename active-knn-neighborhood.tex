...
Fix the learning rate $\eta$ and the abstaining interval $\Delta$.

As $\X$ is finite we have a finite set of possible
balls (recall that a ball is identified with the subset of $\X$ that
falls inside the ball).

Each ball defines two prediction functions $E_{x,r,+1}$ and
$E_{x,r,-1}$, defined as
\[
  E_{x,r,s}(z) = \begin{cases}
    s & \mbox { if } z \in B(x,r)\\
    0 & \mbox{ Otherwise }
  \end{cases}
\]

When predicting on a point $x$ we consider not just the ball centered
at $z$, which is what we did in the AKNN paper. Instead, we consider
balls the contain $x$ but are not centered on it.

Specifically a point $x$ and a radius $R$, define a set of {\em local predictors} $\E(x,R)$ as
follows. Let $B(z,r)$ be a ball such that $B(z,r) \subseteq B(x,R)$
and such that $x \in B(z,r)$. $B(z,r)$ corresponds to the two predictors
$E(z,r,+1)$ and $E(z,r,-1)$. $\E(x,R)$ contains all prediction
function defined in this way.

\newcommand{\emperr}{\hat{\epsilon}}
\newcommand{\emplog}{\hat{\ell}}

Suppose we have a set of labels. Using these labels we can compute the
empirical error of each prediction function $f=E(x,r,s)$. we use
$\emperr(f)$ to denote this empirical error. (You can think of it as
the average over all labeled examples, where the contribution outside
the ball is zero).

Given the empirical errors we can compute the log ratio for a
neighborhood of radius $R$:

\[
  \emplog(x,R) = \frac
  {\sum_{E_{z,r,+} \in \E(x,R)}e^{-\eta \emperr(E_{z,r,+})}}
  {\sum_{E_{z,r,-} \in \E(x,R)}e^{-\eta \emperr(E_{z,r,-})}}
\]

Using the parameter $\Delta$ as in the exponential weights scheme, we
get a prediction for each radius $R$:
\[
  p(x,R) = \begin{cases}
    +1 & \mbox{ if } \emplog(x,R) > \Delta \\
    0 &  \mbox{ if } |\emplog(x,R)| \leq \Delta \\
    -1 & \mbox{ if } \emplog(x,R) < - \Delta
  \end{cases}
\]

Finally, we combine the predictions for a fixed $x$ over all $R$ by
using the smallest $R$ for which $p(x,R)$ is not zero. Formally:
define $R_x^* = \min\{R \mbox{ s.t. } p(x,R) \neq 0\}$, then $p(x)
\doteq p(x,R_x^*)$

This scheme associates with each unlabeled example $x \in \X$ a {\em
  sequence of log-ratio values}, each associated with a differet
neighborhood size $R$. As in the exponenti 


for $s \in \{-1,+1\}$. Each expert predicts $s$ in the ball $B(z,r)$
and predicts $0$ (i.e. abstain) on the complement of $B(z,r)$.

Points are queried in phases. In phase $t$ the points are selected
uniformly at random from a mixture of two distributions: the uniform
distribution over all of $\X$ (the global sample at phase $t$),
and uniformly at random from $\X_t \subseteq \X$ (the active sample at
phase $t$) 

To estimate the error of an expert $\E(z,r,s)$ we use only samples
that were done on a sample whose support contains $\E(z,r,s)$, in
other words, all glbal samples and active samples where $\X_t
\supset B(z,r)$. Using the combination of these samples we estimate $\hat{\epsilon}(B(z,r))$


\section{Quantifying advantage}
