...
In \emph{nonparametric active learning}, the starting point is a data
set whose labels are hidden but can be obtained individually for a
price. The goal is to label the data set, or to find a corresponding
...
$\X$.~\footnote{Our approach works for any topological space
  that defines a sufficiently rich set of neighborhoods. For concreteness, we
  focus on metric spaces and take the neighborhoods to be balls.}  We can
...
universal and works for any $X$ and $\eta(x)$. Our {\em query
  complexity} bound depends on $X$ and $\eta$, but has no explicit dependence on the smoothness of $\eta(x)$. Instead it depends on
properties of $\eta(x)$ close to the decision boundary where $\eta(x)$ changes sign. In particular, the algorithm is effective even when $\eta(x)$
is discontinuous.
...
\begin{figure}
\begin{center}
\includegraphics[width=3.25in]{figures/ideas.pdf}
\end{center}
...
\paragraph{1. Where to query?}\label{challange:wheretoquery}
...
\paragraph{2. No smoothness assumptions} \label{challange:nosmoothness}

  As no assumptions of smoothness are made, the sign of $\eta(\cdot)$
  can change in an arbitrarily small neighborhood of any given point: the
...
\paragraph{3. Managing sampling} \label{challange:managingsampling}
...
Existing work on nonparametric active learning yields algorithms that are guaranteed to work when the underlying conditional expectation $\eta(x)$ is smooth in some way. Additional assumptions are often needed.
...
we follow the second strategy because the bias of a region
...
On the other hand, the only reliable and
...
The idea of predicting with balls of multiple sizes is a central theme of \cite{BDFM19}, which predicts at a point $x$ by choosing a ball centered at $x$ that is as small as possible while having a clear bias towards one label. That work does not consider active learning, which demands further innovation. For instance, to assess the uncertainty at a point $x$, we look at balls \emph{containing} $x$, not just \emph{centered} at $x$ (Fig.~\ref{fig:challenges}(b)).
...
(average $\eta$ values) are easier to estimate. The overall querying
strategy begins with balls at the lowest levels.
%main loop of the
%simplified learning algorithm iterates over the levels $\ell=0,1,2,\ldots$

For any point $x \in \X$, let $\B(x) \subset \B$ denote the collection of
...
follow at a first read.  We therefore start with a
simplified version of the algorithm (Fig~\ref{alg:simple}) that serves
to explain the main ideas but is missing some details.  
The detailed algorithm is described in
Section~\ref{sec:detailedalgorithm}.
...
%\begin{center}
\vspace{.1in}
\noindent\textbf{Repeat for $\ell=0,1,2,\ldots,$:}
\begin{enumerate}[leftmargin=0.6cm]
\item \label{step:Uell} {\bf Compute uncertainty region:}\\ If $\ell=0$, set $U_0 = X$; otherwise set $U_\ell = \{x \in X: \yh_{\ell-1}(x) = \,!\}$ %, \ \yh_\ell(x) = \bot\}$
\item \label{step:QS} {\bf Compute focused query region:}\\ 
The balls to be queried are $R=  \bigcup_{x \in U} \B_\ell(x)$
and the focused query region is their union, $S=\bigcup_{B \in R} B$
...
\textbf{Repeat until} number of queries in each ball $B \in R$ is at least $C/\gamma^2$
  ...
  \item Select a {\tt query} point uniformly at random from $S$
  \end{itemize}

\item \label{step:balls} {\bf Update ball labels:} For each ball $B \in R$, let 
    %$k(B)$ be the number of queries (of any kind) made inside $B$ and let
...
sampling process, the fourth and fifth update the labels of balls
...
according to level $\ell-1$ is ! or ``known unknown''.
%and the label
%according to level $\ell$ is $\bot$ or ``not enough information''.
These are the points for which we need better estimates.

In step~\ref{step:QS} the algorithm identifies balls that need to
be queried to get more information about $U_\ell$. These are
the balls in $\B_\ell$ that contain at least one point of $U_\ell$. 
Region $S$ is the union of these balls. It is the set
...
Step ~\ref{step:balls} assigns a label $\yh(B)$ to each ball $B$ in the focus region. The
label is one of $\{-1,+1,0\}$.  Labels $+1$ and $-1$ correspond to
a confident statement that $\eta_X(B)>0$ and $\eta_X(B)<0$
respectively; here $\eta_X(B)$ is the average value of $\eta(x)$ over points $x \in B \cap X$. The label $0$ indicates that the sign of $B$
...
specific point $x$. Thus in addition to focused (active) querying, we also
...
\paragraph{Managing sampling.} As described in the introduction, we would
like to use queries that fall in the intersection of two balls for estimating
the bias in each of the balls. To facilitate query re-use, it is helpful to 
use Poisson sampling, as described in Section~\ref{sec:poisson}.
...
query the part of it we know. As a result, we are repeatedly in situations where the current region of uncertainty---in particular, the balls $B$ whose labels $\yh(B)$ we wish to determine---already contains some labels from earlier queries, and we wish to reuse those labels in a manner that is statistically sound. We manage this through \emph{Poisson
  sampling}~\cite{ghosh2002sampling} (Fig.~\ref{alg:sampling}).

As an example of what we are able to achieve, suppose we want to determine the label of a ball $B$ at level $\ell$ and have already queried points in several other balls, at levels $\leq \ell$, that intersect $B$. We will be able to apply \emph{all} these queries that fall in $B$ towards estimating $B$'s bias.

Here is the first key idea. In order to infer the label $\yh(B)$ of a ball $B$, we need to estimate the average bias of the ball, $\eta_X(B)$, within $\gamma$, which requires querying roughly $k = \tilde{O}(1/\gamma^2)$ random points in the ball. Instead of picking $k$ points at random in $B$, we query every point in $B$ with probability $\approx k/|X_B|$, \emph{independently}. More precisely, if $B$ lies at level $\ell$ then it has roughly $n/2^{\ell+1}$ points, so we query every point in $B$ with probability $\tau_\ell = \min(2^{\ell+2}k/n, 1)$.


The second key idea is to manage this independent sampling in a manner that makes it easy to reuse earlier queries.  At the outset of the algorithm, each instance $x \in X$ is assigned a random draw from the uniform distribution over $[0,1]$, denoted $T_x$. To query each point in a ball $B$ with probability $\tau_\ell$, we simply select all points in $x \in B$ that have $T_x \leq \tau_\ell$. If a point $x$ was queried earlier---because it also belongs to a ball $B'$ at level $\ell' \leq \ell$, say---then it will now get reused (because $T_x \leq \tau_{\ell'} \implies T_x \leq \tau_\ell$).

%Points are sampled in the order of increasing $T_x$ under some
...
Once a point is queried, it is never queried again; but see the note at the end of Section~\ref{sec:discrete} about situations in which it is permissible to query a point multiple times.
...
\begin{itemize}
\item $U_0 = X$ and $U_\ell = \emptyset$ for $\ell \geq 1$
\end{itemize}
%\vspace{.1in}
...
We now analyze the active learning procedure in a setting where $X \subset \X$ is an arbitrary set of $n$ points. We make no distributional assumption on how $X$ is generated.

\subsection{Accuracy of bias estimates}

Fix the set of balls $\B$ and let $0 < \delta < 1$ be a
predefined confidence parameter. We start with a uniform guarantee on the bias estimates for all balls $B \in \B$.
...
For any $B \in \B$, let $\eta_X(B)$ denote $\mbox{mean}(\{\eta(x): x \in X_B\})$. We say bias estimate $\yh(B) \in \{+1,-1,0\}$ is \emph{$\gamma$-accurate} if:
\begin{itemize}
...
Recall that $\PL_\ell(x) \subset \{-1,+1\}$ denotes possible labels for $x$ given information from balls at levels $\leq \ell$, and is used to assign a value $\yh_\ell(x) \in \{-1,+1,0,!\}$. Despite randomness in querying, it is possible to define two critical levels for each $x$: a level $L_1(x)$ by which $\PL_\ell(x)$ reliably contains the correct label of $x$, and a level $L_2(x)$ by which $\PL_\ell(x)$ reliably omits the wrong label. Intuitively, background sampling dominates until level $L_1(x)$ is reached while focused sampling dominates between $L_1(x)$ and convergence at $L_2(x)$.
Therefore, roughly speaking, the query complexity for point $x$ is low when $L_2(x)-L_1(x)$ is large relative to $L_1(x)$. The precise statement is in Theorem~\ref{thm:queryComplexity}.

\begin{defn}[Critical levels $L_1,L_2$]
Pick any $x \in X$ with $\eta(x) \neq 0$ and let $s(x) = \sign(\eta(x))$. Define $L_1(x)$ to be the smallest level $\ell$ containing some $B_o \in \B_\ell(x)$ for which
\begin{itemize}[leftmargin=0.5cm]
...
A common intuition about active learning is that successive queries gradually constrain the location of the decision boundary. Let's consider the state of affairs when all balls at level $\leq \ell - 1$ have been sampled. The known-unknowns from this level are points $x$ with $\yh_{\ell-1}(x) = \, !$; by Lemma~\ref{lemma:boundary}(b), such points have $L_2(x) \geq \ell$. Focused sampling at level $\ell$ is restricted to balls that contain these points. We will think of this as the \emph{boundary set} at level $\ell$.
...
\end{thm}
Let's interpret this bound. First, recall that levels $\ell$ range from $0$ to $\log (n/k)$. In the many scenarios we later consider, $L_1(x)$ is a constant, so the second term in the bound dominates. The benefit of active learning is captured by the critical level $L_2(x)$ and, most importantly of all, the size of the boundary, $|\Delta_\ell|$, at each level. If $|\Delta_\ell|$ is always $n$, we might end up querying all $n$ points. But in the cases we study, either $|\Delta_\ell| = 0(1)$, in which case we get label complexity polylogarithmic in $n$, or $|\Delta_\ell| = n/{2^{\alpha \ell}}$, $\alpha > 0$, which yields label complexity $\widetilde{O}(n^{1-\alpha})$.

Now let's see how the bound arises. For a specific $x \in X$, there is no telling how $\yh_\ell(x)$ will behave at levels $\ell < L_1(x)$. But for $\ell \geq L_1(x)$, either $\yh_\ell(x) = \, !$, in which case $x$ is in the uncertainty region, or the label of $x$ is correctly set once and for all, which occurs by level $L_2(x)$. 
...
In particular, $O(k \cdot 2^\ell)$ background queries give the biases of all balls in $\B_{\leq \ell}$; this is the first term in $m_o(x)$. And there are $O(k \cdot 2^\ell \cdot |\Delta_\ell|/n)$ focused queries at any level $\ell$: the second term.
...
\end{thm}
If $n^+, n^- = \Theta(n)$, that is, a constant fraction of the points lie to the left of $\lambda_L$ and to the right of $\lambda_R$, the label complexity is $O(\log^2 n)$. If in addition $r^+, r^- = \Theta(n)$, that is, a constant fraction of the points lie between $\lambda_L, \lambda_R$ and the decision boundary, the label complexity is $O(\log n)$. 
...
\section{The distributional setting}

In Section~\ref{sec:continuous} (appendix), we consider the familiar scenario where $X$ is drawn from a distribution $\mu$ on $\X$. We show how Theorem~\ref{thm:label-complexity} implies label complexity bounds based on curvature and smoothness properties of the boundary (Theorem~\ref{thm:label-complexity-specific}). This recovers minimax bounds obtained earlier for active learning under smoothness~\cite{CN08}.

A key component of the analysis is a new probabilistic notion of distance. We take the distance from a point $x$ to a set $S$ to be the probability mass of the smallest ball in $\B$ that contains $x$ and touches $S$, that is, $d(x,S) = \inf\{\mu(B): B \in \B(x), B \cap S \neq \emptyset\}$. We then find, for instance, that the second critical level $L_2(x)$ of any point $x$ is easily bounded in terms of its distance to the set of points with the opposite label (Lemma~\ref{lemma:L2-bound}). Other relevant quantities such as boundary size $|\Delta_\ell|$ also have simple expressions in terms of this distance. See the appendix for further details.
...
any neighborhoods can be used. Although this generality is attractive, it leaves
...
complexity of the algorithm could be high. Future work will be directed
...
\onecolumn
\begin{center}
\LARGE{Nonparametric active learning without smoothness\\
