...
Instances lie in a metric space $(\X, d)$, with labels in $\Y = \{-1,+1\}$. The label of $x \in \X$ is drawn according to the conditional probability function
$$ \eta(x) = \E[Y | X=x] = \pr(Y=1|X=x) - \pr(Y=-1|X=x) = 2 \pr(Y=1|X=x) - 1 .$$
The \emph{Bayes-optimal classifier} on $\X$ is $g^*(x) = \sign(\eta(x))$.

The task we wish to solve is \emph{finite population annotation}: assigning labels to a finite dataset $X \subset \X$ of $n$ points. We are given the ability to query the label of any point $x \in X$, which will be drawn according to $\eta(x)$. We are also given parameters $0 < \gamma, \delta < 1$.
...
\item[(a)] \emph{Points within a ball.} For any $B \in \B$, let $X_B$ denote the points in $X$ that can be used to assess the bias of $B$. This is simply $X_B = X \cap B$ if $\B$ is a predefined set of balls. But if $\B$ is constructed in a way that depends on $X$ then $X_B$ should exclude points in $X \cap B$ that cannot be considered random samples from $B$.
\yoav{Would it be a significant problem to restrict everything to a
  pre-defined set of balls? That would keep things simpler.}
...
\item[(c)] \emph{Balls used to evaluate a given point.} For any
  $x \in \X$, let $\B(x) \subset \B$ denote the collection of balls
  that will be used in determining $x$'s label. There are many ways in
  which this could be set, for instance, (i) all balls containing $x$,
  or (ii) all balls centered at $x$. As before, we partition these
  balls by sampling-level, so that $\B_\ell(x) = \B(x) \cap \B_\ell$.
  \yoav{Seems to be switching back and forth between using the whole
    space $\X$ and the set of instances $X$.}
\end{enumerate}
...
\item[(b)] \emph{Possible labels for a point.} Pick any $x$ and any level $\ell$. Once bias-estimates are available for all balls $B \in \B_\ell(x)$, the possible labels for $x$ at level $\ell$, denoted $\PL_\ell(x) \subset \{-1,+1\}$, are defined as follows:
\begin{gather}
...
$$ S =  \bigcup_{x \in U} \bigcup_{B \in \B_\ell(x)} X_B $$
