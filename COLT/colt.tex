...
\begin{minipage}[t]{5.9in}
  ...
  \begin{itemize}
    ...
    \end{itemize}
    \vspace{1cm}
    $\learn(n)$  learn from $2n$ queries\\

    \begin{itemize}
    \item For each $x \in X$
      \begin{itemize}
      \item Let $\ell(x)$ be the maximal level $\ell$ such that
        $\yh_\ell(x) \in \{-1,0,+1,!\}$ or $\infty$ if no such level
        exists
      \item if $\yh_\ell(x) = !$ the add $\bigcup \B_{l+1}(x)$ to $S$
      \end{itemize}
      \item query $S$ $2n$ times. \yoav{I realize that there might be some
        gain from updating $S$ after each query.}
\end{itemize}
\end{minipage}}
