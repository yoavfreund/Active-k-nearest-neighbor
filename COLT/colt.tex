...
\item[(c)] \emph{Balls used to evaluate a given point.} For any $x \in \X$, let $\B(x) \subset \B$ denote the collection of balls that will be used in determining $x$'s label. There are many ways in which this could be set, for instance, (i) all balls containing $x$, or (ii) all balls centered at $x$. As before, we partition these balls by sampling-level, so that $\B_\ell(x) = \B(x) \cap \B_\ell$.
...
\end{equation}
For any $B \in \B_\ell$, define its \emph{query-set} to be $\Gamma(B) = \{z \in X_B: T_z \leq \tau_\ell\}$. We base our bias-estimate for $B$ on the labels of points in $\Gamma(B)$. The following is a consequence of Lemma~\ref{lemma:large-deviation-bounds} in the appendix.
\begin{theorem}
