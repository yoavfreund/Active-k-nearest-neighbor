...
\item[(c)] \emph{Balls used to evaluate a given point.} For any
  ...
  that will be used in determining $x$'s label. Similarly $\B_\ell(x)
  \subset \B_\ell$ is the collection of balls from level $\ell$ that
  contain $x$.
...
\begin{itemize}
    \item   $\predict(x)$\\
  \begin{itemize}
    \item Let $\ell(x)$ be the maximal level $\ell$ such that $\yh_\ell(x) \in
      \{-1,0,+1\}$ or $\infty$ of no such level exists
    \item If $\ell(x) < \infty$ then return $\yh_{\ell(x)}(x)$
    \item if $\ell(x) = \infty$, return !
    \end{itemize}
    \item
    $\learn(\ell)$  query to complete level $\ell$\\
    Repeat until no conflict
    \begin{itemize}
        \item Set the query region for level $\ell$.
        \item Sample query region until all balls have $k$ queries. Let the number of queries be $m$
        \item Sample $m$ points from the uniform distribution
        \item Fillin if needed
    \end{itemize}
    \item
    {\bf Fill-in}: When a known-unknown is detected at a level lower than $\ell$: we need to fill in the conflicted balls up to level $\ell$.
    \end{itemize}
\end{minipage}}
