...
In our model, all instances lie in a metric space $(\X, d)$. Each $x \in \X$ has an associated label $y \in \Y = \{-1,+1\}$, drawn according to an unknown conditional probability function given by
...
At the outset, we are given a finite dataset $X \subset \X$ of $n$ points, and we have the ability to query the labels of any of them. We are also given parameters $0 < \gamma, \delta < 1$. We want a procedure that chooses the next point, or batch of points, to query. This process will be applied repeatedly and then stopped at an arbitrary time, whereupon labels $\yh(x)$ must be provided for all $x \in X$, including those that were queried. Ideally, these will equal the Bayes-optimal labels $g^*(x)$, but we will only be judged on points $x \in X$ with $|\eta(x)| \geq \gamma$. That is, the number of mistakes is taken to be:
...
In the active learning algorithm, sampling is organized around a collection $\B$ of subsets of $\X$. These are the atomic sets on which we assess label bias and we call them ``balls''.
...
We put $B$ at {\it level} $\ell \geq 0$ if
\begin{equation}
...
For any $x \in \X$, let $\B(x) \subset \B$ denote the collection of balls that contain $x$ and can thus be used in determining $x$'s label. As before, we partition these balls by sampling-level, so that $\B_\ell(x) = \B(x) \cap \B_\ell$.

\subsection{Estimating bias}

We use label-queries to get qualitative estimates of the biases of balls $B \in \B$. These are in turn used to estimate the labels of individual points.
...
\end{itemize}
This is spelled out precisely in the algorithm of Figure~\ref{alg:main}. The label-estimate for $x$ at level $\ell$, denoted $\yh_\ell(x)$, is then defined to be $-1$ if $\PL_\ell(x) = \{-1\}$, or $+1$ if $\PL_\ell(x) = \{+1\}$, or $0$ if $\PL_\ell(x) = \{\}$, or $!$ if $\PL_\ell(x) = \{-1,+1\}$, and remains fixed thereafter. Points labeled $!$ are ``known unknowns'': they are near the decision boundary and need further investigation. Points labeled $0$ show no significant bias.

\subsection{The algorithm}

The overall active learning algorithm is shown in Figure~\ref{alg:main}. It makes calls to subroutines for background and focused sampling, shown in Figure~\ref{alg:sampling}. 
...
We'll start with a uniform guarantee on the bias estimates for all balls $B \in \B$. Take $0 < \delta < 1$ to be a predefined constant, and define
\begin{equation}
c_1 = \sqrt{3 \ln \frac{4|\B|}{\delta}} .
\label{eq:c1}
\end{equation}
Recall from Figure~\ref{alg:sampling} that for any level $\ell$, threshold $\tau_\ell$ is given by
\begin{equation}
\tau_\ell = \min \left( \frac{2^{\ell+2}k}{n}, \ 1 \right)
\label{eq:threshold-ell}
\end{equation}
The choice of $k$ is determined by the following result, proved in the appendix.
\begin{lemma}
Suppose that $k \geq (8c_1/\gamma)^2$ where $c_1$ is given in (\ref{eq:c1}). 
...
The quantities $\yh_\ell(x)$ and $\PL_\ell(x)$ depend upon the random choices of the querying algorithm. Nevertheless, it is possible to define two critical levels for each $x$: a level $L_1(x)$ by which $\PL_\ell(x)$ will reliably contain the correct label of $x$, and a level $L_2(x)$ by which $\PL_\ell(x)$ will reliably omit the wrong label.
...
For any level $\ell$, define the \emph{boundary set} at that level to be
\begin{equation}
\Delta_\ell 
...
In order to apply Theorem~\ref{thm:label-complexity-dist}, we need to be able to bound two quantities: the levels $L_1(x)$ and $L_2(x)$; and the probability masses of boundary sets $\partial_{p,q}$. To do this, we now introduce two simplifying assumptions.
 ...
 Let $\X_\gamma = \{x \in \X: |\eta(x)| \geq \gamma\}$; thus $X \cap \X_\gamma$ is the set of points we want labeled. We will further divide this set by label: for $s \in \{-1,+1\}$, let $\X^s_\gamma = \{x \in \X: s \cdot \eta(x) \geq \gamma\}$. 
\begin{enumerate}
...
\begin{enumerate}
\item[(C1)] [Strong density condition] Distribution $\mu$ on $\X \subset \R^d$ admits a density that is bounded below and above: there exist constants $c_o, c_1 > 0$ such that for all balls $B \in \B$,
...
Recall the definitions of the constant $c_1$ from (\ref{eq:c1}) and of the thresholds $\tau_\ell$ from (\ref{eq:threshold-ell}). For any $B \in \B_\ell$, define its \emph{query-set} to be $\Gamma(B) = \{z \in X_B: T_z \leq \tau_\ell\}$. We base our bias-estimate for $B$ on the labels of points in $\Gamma(B)$.
...
\begin{cor}
Suppose that $k \geq (8c_1/\gamma)^2$. For each $B \in \B$, let $\widehat{\eta}(B)$ be the average label on the query-set $\Gamma(B)$, and define the bias-estimate $\yh(B)$ as follows:
$$ \yh(B)
= 
\left\{
\begin{array}{ll}
